...
int main() {
    ...
    int cmds_n = 0; // Number of commands
    int tokens_n[MAX_COMMANDS] = {0}; // Array for number of tokens per command
    ...
    // Allocating buffer for command input, command pointers and command token pointers
    s = (char *)malloc(mem_s);
    tokens = (char ***)malloc(mem_cmds);
    for(i = 0; i < MAX_COMMAND_TOKENS; i++) {
        tokens[i] = (char **)malloc(mem_tokens);
    }

    // Checking for failed mallocs
    ...
    while (((*(tokens+cmds_n) = strsep(&s_buf, PIPE_DELIM)) != NULL) && cmds_n <= MAX_COMMANDS) {
        cmds_n++;
    }
    ...
    for (i = 0; i < cmds_n; i++) {
        ...
        char *tok_i = tokens[i];
        while ((*(*(tokens[i])+tokens_n[i]) = strsep(&tok_i, " ") != NULL) && (tokens_n[i] <= MAX_COMMAND_TOKENS)) {
            tokens_n[i]++;
        }
