...
int main(int argc, char *argv[]) {
    if (argc >= 2) {
        int proc = get_nprocs_conf() - 1; // # of configured processors
        int err, fd, i, b, t, blocks, offset; 
        ...
        // Ensure there should be at least 1 consumer thread created, even on single-threaded machines
        if (proc == 0) proc = 1;
        ...
        for (i = 1; i < argc; i++) {
            ...
            for (b = 0; b < src_stat.st_size; b += BLOCK_SIZE) {
                ...
                if ((b + BLOCK_SIZE) <= src_stat.st_size) {
                    offset = 0;
                    while(*(src + BLOCK_SIZE + offset) == *(src + BLOCK_SIZE + offset + 1)) {
                        offset++;
                        fprintf(stderr, "offset!\n");
                    }
                    thread_info->block->ptr = src + b + offset;
                    thread_info->block->size = BLOCK_SIZE + offset;
...
void *consumer(void *param) {
    ...
    // Allocating uint and char buffer
    mem_buf = (sizeof(uint32_t)+sizeof(char))*BLOCK_SIZE;
    buf = malloc(mem_buf);
    ...
    while(1) {
        ...
        // Write buffer to stdout, iterate current block (atomically)
        if (write(fileno(stdout), buf, buf_i) == -1) {
            perror("pzip: buffer write in thread failed");
            pthread_exit(NULL);
        }
        block_cur++;
